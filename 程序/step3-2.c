...
int32_t err_step = 0;
int8_t is_fist_rec = 1;
static void twai_receive_task(void *arg)
{
    while (1) {
        twai_message_t rx_msg;
        twai_receive(&rx_msg, portMAX_DELAY);
        if (rx_msg.identifier == 0x0B2) 
        {   
            int32_t data = 0;
            for (int i = 0; i < 4; i++) {
            ...
            if(is_fist_rec)
            {
                err_step = data+tar_pul;
                is_fist_rec = 0;
            }
            tar_pul = err_step - data;
            gpio_set_level(LED_OI, 1);
        }
        else if (rx_msg.identifier == 0x0C3)
        {
...
static void run_task(void *arg)
{
    ...
    while (1) {
        // twai_message_t tx_msg = {.data_length_code = 8, .identifier = 0x0A1};
...
//    gpio_pad_select_gpio(PIN_NUM_CS);                // 选择一个GPIO
	...
	ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, 800);
